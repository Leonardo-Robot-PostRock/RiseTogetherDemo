...
| RF-38 | Calcular match score (0-100) con pesos explícitos: skills exactos 35%, transferibles 15%, emergentes 10%, experiencia 15%, trayectoria_aprendizaje 10%, proyectos 15%. La educación formal no es requisito sino señal compensable. Peso máximo de educación: 10%. | 🔴 | Matching | Servicio de dominio. Extiende RF-38 anterior con pesos explícitos y factores de fair matching. |
| RF-38-EXT | Sistema de niveles de cualificación del candidato (qualification_levels): NONE, STUDENT, ADVANCED_STUDENT, PARTIALLY_QUALIFIED, FULLY_QUALIFIED, LICENSED. Cada nivel tiene peso en matching (50%-100%). La falta de educación formal se compensa con proyectos, experiencia y consistencia. | 🔴 | Matching | Catálogo normalizado en tabla `qualification_levels`. |
| RF-38-LEGAL | Separación de capacidad legal vs operativa para profesiones reguladas: legal_requirement (el puesto requiere firma/responsabilidad legal) y operative_capacity (el candidato puede realizar las tareas técnicas). Un candidato puede tener operative=true pero legal=false (ej: trabajo bajo supervisión). | 🔴 | Matching | Campos en `jobs` y lógica de matching. |
...
| ID | Requisito | Prioridad | BC | Observaciones |
|----|-----------|-----------|-----|---------------|
| RF-48 | Candidato registra años de experiencia total, salary deseado, preferencia de remoto, nivel de cualificación primario (qualification_level), y si su área requiere licencia profesional | 🔴 | Profiles | Columnas en `candidates`. Extiende RF-48 anterior. |
| RF-49 | Candidato puede agregar proyectos/portafolio como evidencia de skills. Es el mecanismo principal de compensación para candidatos sin educación formal. | 🔴 | Profiles | Tabla `candidate_projects`. Prioridad cambiada de 🟡 a 🔴. |
...
#### `jobs` — Agregar FK a employer, campos de promoción y profesiones reguladas
...
| `legal_requirement BOOLEAN DEFAULT FALSE` | ADD | 🔴 | TRUE si el puesto requiere capacidad legal (firma, responsabilidad) |
| `requires_license BOOLEAN DEFAULT FALSE` | ADD | 🔴 | TRUE si el puesto requiere licencia profesional vigente |
...
#### `candidates` — Enriquecer perfil para matching + fair scoring

| Columna | Cambio | Prioridad | Notas |
|---------|--------|-----------|-------|
| `years_of_experience SMALLINT` | ADD | 🔴 | Experiencia laboral total |
| `desired_salary_min DECIMAL(15,2)` | ADD | 🟡 | Salario deseado mínimo |
| `desired_salary_max DECIMAL(15,2)` | ADD | 🟡 | Salario deseado máximo |
| `desired_employment_type VARCHAR(20)` | ADD | 🟡 | Tipo de empleo preferido |
| `open_to_remote BOOLEAN DEFAULT FALSE` | ADD | 🟡 | Dispuesto a trabajar remoto |
| `primary_qualification_level VARCHAR(30)` | ADD | 🔴 | FK a qualification_levels.level_code. Nivel más alto de cualificación |
| `license_required_for_role BOOLEAN DEFAULT FALSE` | ADD | 🟡 | TRUE si su área requiere licencia profesional |
...
#### Matching — Catálogo de niveles de cualificación (Fair Scoring)

```sql
-- Catálogo de niveles de cualificación para fair matching
CREATE TABLE qualification_levels (
    level_code VARCHAR(30) PRIMARY KEY,  -- PK: NONE, STUDENT, ADVANCED_STUDENT, PARTIALLY_QUALIFIED, FULLY_QUALIFIED, LICENSED
    name VARCHAR(50) NOT NULL,           -- Display name
    description TEXT NOT NULL,           -- Descripción del nivel
    weight_percentage INT NOT NULL,      -- Peso en matching (0-100)
    sort_order INT NOT NULL              -- Para UI
);

-- Datos iniciales del catálogo
INSERT INTO qualification_levels (level_code, name, description, weight_percentage, sort_order) VALUES
('NONE', 'Sin formación formal', 'Sin educación formal documentada. Puede compensar con proyectos, experiencia o certificaciones.', 50, 1),
('STUDENT', 'Estudiante', 'Estudiante cursando carrera o formación. Menos del 50% completado.', 60, 2),
('ADVANCED_STUDENT', 'Estudiante avanzado', 'Estudiante con más del 50% de la carrera completada o en etapa final de formación.', 75, 3),
('PARTIALLY_QUALIFIED', 'Parcialmente cualificado', 'Título incompleto, certificaciones parciales, título extranjero sin homologar, o formación equivalente.', 85, 4),
('FULLY_QUALIFIED', 'Completamente cualificado', 'Título completo o equivalente reconocido oficialmente.', 90, 5),
('LICENSED', 'Licenciado', 'Licencia profesional vigente para profesiones reguladas. Capacidad legal total.', 100, 6);

-- Educación del candidato (referencia al catálogo)
CREATE TABLE candidate_education (
    id CHAR(36) PRIMARY KEY,
    candidate_id CHAR(36) NOT NULL,
    qualification_level VARCHAR(30) NOT NULL DEFAULT 'NONE',
    ...
    FOREIGN KEY (qualification_level) REFERENCES qualification_levels(level_code)
);

CREATE INDEX idx_candidate_education_candidate ON candidate_education(candidate_id);
CREATE INDEX idx_candidate_education_level ON candidate_education(qualification_level);
```
...
| **V29** | Sistema de fair matching: Tabla `qualification_levels` (catálogo), `candidate_education`. ALTER `candidates`: ADD `primary_qualification_level`, `license_required_for_role`. ALTER `jobs`: ADD `legal_requirement`, `requires_license`. | 🔴 | Niveles de cualificación, separación capacidad legal vs operativa, proyectos como compensación. |
