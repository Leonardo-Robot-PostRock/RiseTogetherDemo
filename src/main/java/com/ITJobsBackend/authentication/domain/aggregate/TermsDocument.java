...
/**
 * Catalog entry for a published version of a legal document (e.g. Terms of Service).
 * Immutable once created — new versions are new aggregate instances.
 */
public class TermsDocument {
    ...
    public static TermsDocument create(TermsType termsType, String version, String content) {
        if (termsType == null) throw new ValidationException("Terms type cannot be null");
        if (version == null || version.isBlank()) throw new ValidationException("Version cannot be blank");
        if (content == null || content.isBlank()) throw new ValidationException("Content cannot be blank");
        return new TermsDocument(TermsDocumentId.generate(), termsType, version, content, Timestamp.now());
    }
    ...
    public TermsDocumentId getId() { return id; }
